...
export class SettlementService {
  ...
  static recalculateSettlements(
    ...
    const resetPayments: any[] = payments.map(p => ({
      ...p,
      settlements: [] as any[],
      unsettledAmount: p.amount
    }));

    const resetInvoices: any[] = invoices.map(inv => ({
      ...inv,
      settlements: [] as any[],
      settledAmount: 0,
    ...
    for (const payment of sortedPayments) {
      ...
      payment.unsettledAmount = payment.amount - (payment.settlements?.reduce((sum: number, s: any) => sum + s.amount, 0) || 0);
    }
