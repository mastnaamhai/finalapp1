...
// Helper to calculate payment info
const calculatePaymentInfo = (invoice: Invoice, payments: Payment[]) => {
  if (!invoice || !payments) return { paidAmount: 0, balanceDue: 0 };
  ...
  const balanceDue = (invoice.grandTotal || 0) - paidAmount;
  return { paidAmount, balanceDue };
};

export const Dashboard: React.FC<DashboardProps> = ({ lorryReceipts, invoices, payments, truckHiringNotes, onViewChange, onDeleteLr, onDeleteInvoice }) => {
  // Debug logging
  ...
  const kpis = useMemo(() => {
    ...
    const outstandingPayments = safeInvoices.reduce((acc, inv) => {
      const { balanceDue } = calculatePaymentInfo(inv, payments || []);
    ...
    }).reduce((acc, thn) => acc + (thn.freightRate || 0), 0);

    const outstandingSupplierPayments = safeTruckHiringNotes.reduce((acc, thn) => acc + (thn.balanceAmount || 0), 0);
  ...
  }, [lorryReceipts, invoices, payments, truckHiringNotes]);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Dashboard"
        subtitle="Welcome to TranspoTruck - Manage your transport operations efficiently"
      />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <KpiCard
          ...
          icon="📦"
          onClick={() => {
            // Encode the array of IDs as a JSON string in the URL
            const params = new URLSearchParams();
            if (unbilledLrIds.length > 0) {
              params.set('ids', JSON.stringify(unbilledLrIds));
            }
            onViewChange({
              name: 'LORRY_RECEIPTS',
              filters: { ids: unbilledLrIds },
            ...
            return (
              <div key={inv._id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  ...
                  <p><span className="font-medium">Balance Due:</span> <span className="text-red-600 font-semibold">₹{balanceDue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></p>
                </div>
                <div className="flex space-x-2 mt-3">
                  <button
