                ...
                <div className="grid grid-cols-12 gap-1 mb-2">
                    ...
                    <div className="col-span-8">
                        ...
                        <div className="grid grid-cols-4 text-center font-bold border-2 border-black bg-white h-[3rem]">
                            <div className="col-span-1 border-r-2 border-black pt-1 pb-1 px-1 flex items-start justify-center text-xs leading-tight">
                                <div>
                                    <div>No. of</div>
                                    <div>Pkgs.</div>
                                </div>
                            ...
                            <div className="col-span-1 pt-1 pb-1 px-2">
                                <div className="h-full flex flex-col">
                                    <div className="h-1/2 border-b-2 border-black flex items-start justify-center text-xs font-bold" style={{ marginTop: '-3px' }}>WEIGHT</div>
                                    <div className="h-1/2 grid grid-cols-2">
                                        <div className="border-r-2 border-black flex items-start justify-center text-xs" style={{ marginTop: '-3px' }}>Actual</div>
                                        <div className="flex items-start justify-center text-xs" style={{ marginTop: '-3px' }}>Charged</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="min-h-[3rem] border-l-2 border-r-2 border-b-2 border-black bg-white">
                            {lorryReceipt.packages && lorryReceipt.packages.length > 0 ? (
                                lorryReceipt.packages.map((pkg, index) => (
                                    <div key={index} className={`grid grid-cols-4 border-b-2 border-black h-[2rem] items-start last:border-b-0 bg-white`}>
                                        <div className="col-span-1 border-r-2 border-black pt-1 pb-1 px-1 text-center h-full flex items-start justify-center text-xs font-medium">{pkg.count || ''}</div>
                                        <div className="col-span-1 border-r-2 border-black pt-1 pb-1 px-1 text-center h-full flex items-start justify-center text-xs font-medium">{pkg.packingMethod || ''}</div>
                                        <div className="col-span-1 border-r-2 border-black pt-1 pb-1 px-1 text-center h-full flex items-start justify-center text-xs font-medium">{pkg.description || ''}</div>
                                        <div className="col-span-1 pt-1 pb-1 px-1">
                                            <div className="grid grid-cols-2 h-full">
                                                <div className="border-r-2 border-black text-center h-full flex items-start justify-center text-xs font-medium">{pkg.actualWeight || ''}</div>
                                                <div className="text-center h-full flex items-start justify-center text-xs font-medium">{pkg.chargedWeight || ''}</div>
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <div className="grid grid-cols-4 h-[2rem] items-start bg-white">
                                    <div className="col-span-1 border-r-2 border-black h-full pt-1 pb-1 px-1"></div>
                                    <div className="col-span-1 border-r-2 border-black h-full pt-1 pb-1 px-1"></div>
                                    <div className="col-span-1 border-r-2 border-black h-full pt-1 pb-1 px-1"></div>
                                    <div className="col-span-1 pt-1 pb-1 px-1">
                                        <div className="grid grid-cols-2 h-full">
                    ...
                    {!hideCharges && (
                        <div className="col-span-4">
                            ...
                            <div className="grid grid-cols-3 text-center font-bold border-2 border-black bg-white h-[2.5rem]">
                                <div className="col-span-2 border-r border-black pt-1 pb-1 px-1 flex items-start justify-center text-xs">RATE</div>
                                <div className="col-span-1 pt-1 pb-1 px-1 text-xs flex items-start justify-center">Amount To Pay</div>
                            </div>
                            {/* Charges Body */}
                            <div className="min-h-[3rem] border-l-2 border-r-2 border-b-2 border-black">
                                {charges.filter(charge => charge.value > 0).map(charge => (
                                    <div key={charge.label} className="grid grid-cols-3 border-b border-black h-[1.8rem] items-start last:border-b-0 bg-white">
                                        <div className="col-span-2 border-r border-black pl-1 pt-1 pb-1 h-full flex items-start text-xs">{charge.label}</div>
                                        <div className="col-span-1 pr-1 pt-1 pb-1 text-right h-full flex items-start text-xs">{charge.value.toFixed(2)}</div>
                                    </div>
                                ))}
                                {/* Show empty row if no charges */}
                                {charges.filter(charge => charge.value > 0).length === 0 && (
                                    <div className="grid grid-cols-3 h-[1.8rem] items-start">
                                        <div className="col-span-2 border-r border-black h-full pt-1 pb-1 px-1"></div>
                                        <div className="col-span-1 h-full pt-1 pb-1 px-1"></div>
                                    </div>
                                )}
                            </div>
                            {/* Charges Total */}
                            <div className="grid grid-cols-3 font-bold border-2 border-black bg-white">
                                <div className="col-span-2 border-r border-black pt-1 pb-1 px-1 text-xs">TOTAL</div>
                                <div className="col-span-1 pt-1 pb-1 px-1 text-right text-xs">{(lorryReceipt.totalAmount || 0).toFixed(2)}</div>
                            </div>
                        </div>
                    )}
                </div>
